...
async def test_update_movie_success(client, db_session, seed_database):
    ...
    response = await client.put(f"/api/v1/theater/movies/{movie_id}/", json=update_data)
    assert response.status_code == 200, f"Expected status code 200, but got {response.status_code}"
...
async def test_update_movie_not_found(client):
    ...
    response = await client.put(f"/api/v1/theater/movies/{non_existent_id}/", json=update_data)
    assert response.status_code == 404, f"Expected status code 404, but got {response.status_code}"
